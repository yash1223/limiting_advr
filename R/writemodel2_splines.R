...
WriteModel2_splines <- function(
  ...
  cat(" 
      ...
      for (t in 1:nyears){
      logit(pi.ct[c,t]) <- a.c[c] + demand.ct[c,t]*beta.c[c] + eta.j[getj.ct[c,t]]
      eta.j[getj.ct[c,t]] <- inprod(alpha.k, B.jk[getj.ct[c,t],])
      }}
      for (r in 1:R){
      ...
      for (k in 2:K){ 
        alpha.k[k] ~ dnorm(alpha.k[k-1], tau.alphak)
      }
