# Instead of this we could just save the process data with the model rds each run .... takes more memory but easier than this

ValProc <- function(modelpath, SSA=FALSE, NOTSSA=FALSE, startyear=1985, endyear=2020){
workdir <- getwd()
...
temp <- readRDS(file.path(modelpath, "mod.rds"))
...
plot_jags_cor1 <- data.frame(a.c, beta.c)
plot_jags1 <- data.frame(yrep.i1, getnc.i, getnt.i, jagsdata$luup.i, getnr.i, demand.i)
return(plot_jags1)
}
