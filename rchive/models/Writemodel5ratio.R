...
WriteModel <- function(
  ...
  cat("
   for (i in 1:n){
    logy.i[i] ~ dnorm(mu.ct[getc.i[i], gett.i[i]], tau.y)
    trueprob.i[i] <- mu.ct[getc.i[i], gett.i[i]]
    y.i[i] ~ dnorm(trueprob.i[i], tauy.i[i])
#    tauy.i[i] <- 1/(2*se.i[i]^2)#samplesize/(trueprob.i[i]*(1-trueprob.i[i]))
    tauy.i[i] <- 1/(2*se.i[i]^2 + nonsamplsd^2)#(trueprob.i[i]*(1-trueprob.i[i]))/samplesize)
    #yrep.i[i] ~ dnorm(mu.ct[getc.i[i], gett.i[i]], tau.y)
   }
nonsamplsd ~ dunif(0,0.1)
  #nonsamplvar <- 1/nonsampl
samplesize~ dunif(0,100)
   tau.y <- pow(.025, -2)
...
Sigma[2,2] <- pow(sigma.beta,2)
      ...
      rho.alphabeta <- 0#~ dunif(-1,1)
      sigma.alpha ~ dunif(0,3)
      sigma.beta ~ dunif(0,3)
...
gamma ~ dnorm(0, 0.1)
    for (c in 1:C){
      for (t in 1:nyears){
        ...
        logitpi.ct[c,t] <- alpha.c[c] + beta.c[c]*sqrt(totaldemand.ct[c,t])
#        logitpi.ct[c,t] <- alpha.c[c] + beta.c[c]*log(1+totaldemand.ct[c,t])
#        logitpi.ct[c,t] <- alpha.c[c] + beta.c[c]*(totaldemand.ct[c,t])+gamma*(totaldemand.ct[c,t])^2
#        logitpi.ct[c,t] <- alpha.c[c] + beta.c[c]*(totaldemand.ct[c,t])
      }
alpha.c[c] <- alphabeta.c2[c,1]
beta.c[c] <- alphabeta.c2[c,2]
#beta.c[c] <- beta1 #alphabeta.c2[c,2]
    alphabeta.c2[c,1:2] ~ dmnorm(alphabeta.r2[getr.c[c], 1:2], InvSigma)
#alpha.c[c] ~ dnorm(0, 0.1)
#beta.c[c] ~ dnorm(0, 0.1)
#alpha.c[c] ~ dnorm(alphabeta.r2[getr.c[c],1], tau.alpha)
#beta.c[c] ~ dnorm(alphabeta.r2[getr.c[c],2] + rho.alphabeta*sigma.beta/sigma.alpha*(alpha.c[c] - alphabeta.r2[getr.c[c],1]),
