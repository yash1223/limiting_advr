...
WriteModelOld <- function(
  ...
  cat("
   ...
   for (j in 1:ntrain){
    y.i[getitrain.j[j]] ~ dnorm(mu.ct[getc.i[getitrain.j[j]], gett.i[getitrain.j[j]]], tauy.i[getitrain.j[j]])
   }
   for (i in 1:n){
...
#    y.i[i] ~ dnorm(mu.ct[getc.i[i], gett.i[i]], tauy.i[i])
    ...
    # for checks
    yrep.i[i] ~ dnorm(mu.ct[getc.i[i], gett.i[i]], tauy.i[i])
    loglike.i[i] <- logdensity.norm(y.i[i], mu.ct[getc.i[i], gett.i[i]], tauy.i[i])
   }
...
#                      gamma*timemin2005.ct[c,t]
#                      gamma*change* (t > timechange)*(t-timechange) 
                      gamma*change* (t > timechange)*log(max(t, timechange+1)-timechange) 
#      logitpi.ct[c,t] <- alpha.c[c] + beta.c[c]* timemin2005.ct[c,t]#(sqrt(totaldemand.ct[c,t])- sqrt(0.5))
#      logitpi.ct[c,t] <- alpha.c[c] + beta.c[c]*change* (t > timechange)*(t-timechange) 
      #        logitpi.ct[c,t] <- alpha.c[c] + beta.c[c]*(logit(totaldemand.ct[c,t]) - logit(0.5))
      }
